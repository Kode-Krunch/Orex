  ...
  try {
    switch (feature) {
      case secTableBottomToolbarFeaturesEnum.INSERT:
        return secTableSelectedRows.length === 0;
...
const getCellClassNames = (tableData, row, arr, ignoreSelectedRow) => {
  ...
  if (row.ContentTypeCode === 14) {
    ...
    if (row.status === 'high') {
      cellClassNames = cellClassNames + ` !bg-emerald-500`;
    } else if (row.status === 'low') {
      cellClassNames = cellClassNames + ` !bg-[#000]`;
    }
    else if (row.IsRecorded === 1 && row.status == 'ORG')
      cellClassNames = cellClassNames + ` !bg-yellow-300`;
    else if (row.IsRecorded === 0 && row.status == 'ORG') {
      cellClassNames = cellClassNames + ` !bg-yellow-300`;
    }
    ...
    const isSameRowPresent =
      tableData.filter(
        (curRow) =>
          curRow.ContentCode === row.ContentCode &&
          curRow.SeasonNo === row.SeasonNo &&
          curRow.EpisodeNo === row.EpisodeNo,
      ).length > 1
        ? true
        : false;
    cellClassNames =
      cellClassNames +
      ` ${isSameRowPresent ? '!bg-cyan-500' : `!bg-${result[0]?.NewColourCode}`
      }`;
  }
  return cellClassNames;
};
//` ${isSameRowPresent ? '!bg-cyan-500' : `!bg-${result[0]?.NewColourCode}`
//      ` ${isSameRowPresent ? `!bg-${result[0]?.NewColourCode}` : `!bg-${result[0]?.NewColourCode}`
  ...
  try {
    ...
    newTableData = [getDummyRow(newTableData[0]), ...newTableData].map(
      (row, index) => ({
        ...
        rowId: getRowId('primary', row, 'ContentCode'),
        rowIndex: index,
        cellClassNames: getCellClassNames(tableData, row, arr, true),
        verify: false,
      }),
    );
    if (isEndTimeValid(newTableData, channelSetting)) {
      ...
      return { status: true, newTableData };
    } else {
      openNotification('danger', 'Insufficient FPC Duration');
      setdata10(ogTableData);
      return { status: false, newTableData };
    }
  } catch (error) {
    throw error;
  }
...
const isFPCAllowedToSaveFn = async (
  ...
  try {
    const result =
      ...
      (isDateBackDate(date) && IS_BACKDATED_FPC_ALLOWED_TO_EDIT) ||
      (!isDateBackDate(date) &&
        (isFeatureAllowedForChannel(featuresEnum.FPC_ALLOWED_TO_SAVE, channel)
          ? true
...
const isEndTimeValid = (tableData, channelSetting) => {
  // let allowedHours = subtractTimes(
  //   channelSetting.EndTime,
  //   `${channelSetting.StartTime}:00`,
  // );
  // let consumedHours = '00:00:00:00';
  // tableData.slice(1).forEach((row) => {
  //   consumedHours = addTimes(
  //     consumedHours,
  //     subtractTimes(`${row.EndTime}:00:00`, `${row.StartTime}:00:00`),
  //   );
  // });
  // return consumedHours <= allowedHours;
  return true;
};
