...
import { Posteventcolor, Puteventcolor } from 'services/MasterService';
...
const validationSchema = Yup.object().shape({
  ...
  EventDefaultFrontColor: Yup.string()
    .min(1, 'Too Short!')
    .max(50, 'Too Long!')
    .required('Event Font Color Required'),
...
const EventColorEdit = forwardRef((props, ref) => {
  ...
  const Addeventcolor = async (values, token) => {
    try {
      const resp = await Posteventcolor(values, token);
      if (resp.status === 200) {
        setlog('success');
  ...
  const Editeventcolor = async (values, token) => {
    try {
      const resp = await Puteventcolor(values, token);
      //console.log(resp)
      if (resp.status === 200) {
        setlog('success');
                  ...
                  <Field name="EventType">
                    {({ field, form }) => (
                      <Input
                        ...
                        onChange={(e) => {
                          const newValue = e.target.value;
                          console.log(/^[a-zA-Z ]+$/.test(newValue));
                          if (/^[a-zA-Z ]+$/.test(newValue)) {
                            form.setFieldValue(field.name, newValue);
                          }
                          if (newValue == '') {
