...
import { setPromoMasterSearchKeyword } from 'store/sectionWiseState/promoMasterSlice';
  ...
  return (
    <span className="flex items-center">
      ...
      <span className="mr-1 font-semibold">
        <InputwithVoice
          value={promoMasterSearchKeyword}
          className=" solid"
          placeholder="Search all columns..."
          size="sm"
          onChange={(e) => {
            dispatch(setPromoMasterSearchKeyword(e.target.value));
          }}
        />
      </span>
      <span className="mr-1 font-semibold">
...
const PromoMaster = () => {
  ...
  const promoMasterSearchKeyword = useSelector(
    (state) => state.promoMaster,
  ).promoMasterSearchKeyword;
  ...
  useEffect(() => {
    (async (values) => {
      const Promotype = await apiGetPromotypedropdown(values);
      const formattedOptions = Promotype.data.map((option) => ({
        value: option.PromoTypeCode,
        label: option.PromoTypeName,
      }));
      setPromotype([{ value: 0, label: 'All Content' }, ...formattedOptions]);
    })();
  ...
  const getPromos = async (search) => {
    try {
      ...
      // Fetch data from API
      const resp = await apiGetPromomaster(params, search);
      if (resp.status === 200) {
        // Handle response based on IsChannelwise flag
        const { Active, InActive, data } = resp.data; // Destructure if channel-wise
        setActivePromos(Active || []); // Default to empty array if not defined
        setISActivePromos(InActive || []); // Default to empty array if not defined

        // Convert integers to strings in promo data
      ...
      console.error('Error fetching promos:', error.message || error); // Enhanced error message
      setMessage('An error occurred while fetching promos.'); // Display error message to user if necessary
    } finally {
      setShowLoader(false); // Hide loader regardless of success or error
    }
  };

  // Helper function to convert integers to strings in promo data
  ...
  const FilterTableData_On_Card = (CardName) => {
    switch (CardName) {
      case IsChannelwise ? 'Total Content' : 'Total Promos':
        if (activePromos + isActivePromos > 500) {
          setIsConfirm(true);
        } else {
          getPromos('%%%%%');
        }
  ...
  function getTableColumnsWithActions() {
    try {
      return [
        ...
        {
          header: 'Segment No',
          accessorKey: 'SegmentNo',
        },
        {
          header: 'Tape Type',
          accessorKey: 'TapeTypeName',
        },
        {
          header: 'Video ID',
          accessorKey: 'VideoID',
  ...
  const [formState, setFormState] = useState({ Duration: [0, 100] });
  const Footer = (
  ...
  const getPromosFilter = async (data) => {
    try {
      setShowLoader(true);
      const resp = await apiGetPromomasterFilter(channel, data);
  ...
  return (
    <>
      ...
      >
        <div className="mb-2">
          <h6>{IsChannelwise ? 'Content Keyword' : 'Promos Keyword'}</h6>
          <Input
            ...
            onChange={(e) => {
              setFormState((prevFormState) => {
                const newState = { ...prevFormState, Caption: e.target.value };
                return newState;
              });
            }}
            size="sm"
        ...
        <div className="mb-2">
          ...
          <Input
            ...
            onChange={(e) => {
              setFormState((prevFormState) => {
                const newState = { ...prevFormState, VideoID: e.target.value };
                return newState;
              });
            }}
            size="sm"
        ...
        <div className="mb-2">
          <h6>{IsChannelwise ? 'Content Type ' : 'Promos Type'}</h6>
          <Select
            placeholder="Select Promo Type"
            value={Promotype.filter(
              (option) => option.value === formState.PromoTypeCode,
            )}
            name="PromoTypeCode"
            options={Promotype}
            onChange={(e) => {
              setFormState((prevFormState) => {
                const newState = { ...prevFormState, PromoTypeCode: e.value };
                return newState;
              });
            }}
            size="sm"
        ...
        <div className="mb-2">
          <div>
            ...
            <div className="flex  justify-between items-center mb-2">
              <p>{formState?.Duration[0]}</p> <p>{formState?.Duration[1]}</p>
            </div>
            <Slider
              value={formState?.Duration}
              onChange={(e) => {
                setFormState((prevFormState) => {
                  const newState = { ...prevFormState, Duration: e.value };
                  return newState;
                });
              }}
              className="w-14rem"
      ...
      >
        ...
        <p>Retrieving all data may take some time. Do you wish to proceed?</p>
            ...
            <div className="flex ">
              <div className="w-9/12 mr-5">
                ...
                <div className="flex mt-5 ml-1">
                  <h5>{TimeCodeDisplay(dialogImage?.PromoDuration)}</h5>
                </div>
                <div className="flex  items-center mt-5">
                  <div
                    style={{
      ...
      >
        ...
        >
          <StatisticCardwithfigure
            ...
            CardNote="Short advertisements aired during breaks in programming."
        ...
        {data && data.length <= 20 && (
          <div className="flex items-center justify-between mb-2">
            <p className="font-medium text-[13px] text-[#ea4343]">
              (Showing Top 20 Records. Use{' '}
              {IsChannelwise ? 'advance filter' : 'search'} for all Records.)
            </p>
            {IsChannelwise && (
              <Button
                variant="solid"
        ...
        <div>
          <ReportsTable
            ...
            externalGlobalFilter={promoMasterSearchKeyword}
            exportFileName={IsChannelwise ? 'StoryMaster' : 'PromoMaster'}
