...
import { FaChromecast, FaRegCopy, FaUserLock, FaUserShield } from 'react-icons/fa';
...
const DisplayTableEmp = ({
  ...
  const logedInUserCode = useSelector((state) => state.auth.session.LoginId);
                ...
                <Tr key={row.id} className="tr">
                  ...
                  {Path == '/UserMaster' ? (
                    ...
                    >
                      ...
                      {/* ${JSON.stringify(row.original.is2FAAllowed)} */}
                      {/* ${JSON.stringify(row.original.LoginCode)}
                      ${JSON.stringify("")} */}
                      {/* {(row.original.is2FAAllowed != false) && (row.original.LoginCode === 64 || row.original.LoginCode === logedInUserCode) && ( */}
                      {(
                        (logedInUserCode === 64) &&
                        (
                          logedInUserCode === row.original.LoginCode ||
                          row.original.is2FAAllowed != false
                        )
                      ) && (
                          <Tooltip title="2FA Auth Remove">
                            <Button
                              size="xs"
                              variant="twoTone"
                              ...
                              icon={<FaUserLock />}
                              onClick={async () =>
                                await apiGet2faremove(row.original.LoginCode)
