...
export const DealDetailsComponentAdd = (
  ...
  const transformedData = {
    Deal: {
      ...
      AgencyPlaceCode: array1.AgencyPlaceCode.value,
  ...
  useEffect(() => {
    if (DataDisplay.length !== 0) {
      const transformedArray = DataDisplay.map((item) => ({
        ...
        Rate: CurrencySysmbol + +item?.Rate,
        TotalSec: [
        ...
        Balance: item?.BalancedSeconds
          ...
          : [
            ...
            .filter(Boolean)
            .join(' '),
        TotalAmount: CurrencySysmbol + +item.Amount,
        UsedAmt: `${CurrencySysmbol} ${item?.ConsumedAmount == 0 || item?.ConsumedAmount == undefined
          ? '00'
          : item?.ConsumedAmount
          }`,
        BalanceAmt: `${CurrencySysmbol} ${item?.BalancedAmount == 0 || item?.BalancedAmount == undefined
          ? item.Amount
          : item?.BalancedAmount
          }`,
        ...(isChannelSelEnabledForDealDetails && {
              ...
              >
                <div className="flex items-center ">
                  <div
                    ...
                    className={` mr-3 ${item.DealLineItemType?.value == 4 ? 'bg-lime-300' : null
