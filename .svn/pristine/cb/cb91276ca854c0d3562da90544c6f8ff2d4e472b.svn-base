...
const DateRangeInputs = ({
  ...
  const formatDate = (date) => {
    ...
    const month = date.toLocaleString('en-US', { month: 'short' }).substring(0, 3);
    return `${day}-${month}`;
  };
  ...
  const updateFormState = (newFormState, currentYear, currentDate, Name) => {
      ...
      if (!key.endsWith('total') && key[0] !== 'Y') {
        ...
        if (parts.length >= 3) {
          ...
          if (dateParts.length === 2) {
            ...
            const updatedKey = `${dealLineItemNo}|${commId}|${day}-${month}-${Name.split('-')[2]}|${formState.MID}|${formState.position}`;
    ...
    for (const key in updatedCopyFormState) {
      updatedCopyFormState[key] = `${key}|${updatedCopyFormState[key]}`;
    }
    const stringFromArray = JSON.stringify(updatedCopyFormState).replace(/[%{}]/g, '');
  ...
  const transformData = (data) => {
    ...
    Object.keys(data).forEach((key) => {
      ...
      if (parts.length === 3) {
        ...
        const valuePart = data[key];
        if (
          typeof valuePart === 'number' ||
          (typeof valuePart === 'object' && valuePart !== null && 'total' in valuePart)
        ) {
          const valueToUse = typeof valuePart === 'number' ? valuePart : valuePart.value;
          if (
            (typeof prefix === 'string' && !prefix.includes('Y') && datePart !== 'total') ||
            typeof prefix === 'number'
          ) {
            const newKey = `${prefix}|${parts[1]}|${datePart}-${new Date().getFullYear()}|${MID}|${position}`;
            result[newKey] = `${newKey}|${valueToUse}`;
          }
        }
      }
    });
  ...
  const handleChange = (
    ...
    const limit = getLimit(DealLineItemTypeCode, NTCBalanceSpots, BalancedSpots, BalancedSeconds);
    const currentItem = checkedItems.find((item) => item.DealLineItemNo === DealLineItemNo);
    const rate = currentItem?.Rate || 0;

    const oldValue = formState['Y' + name]?.value || 0;
    const oldTotal = oldValue * DurInSec;
    const oldCost = (((oldTotal) / 10) * rate);

    if (value === '') {
      removeFormStateEntry(name, totalKey, currentDate, Name);
      if (isPaymentCheck && oldCost > 0) {
        setAvailableAmt((prevAmt) => prevAmt + oldCost);
      }
    } else {
      if (!isNumbers(value)) return;
      const valueInInt = parseInt(value, 10);
      if (isNaN(valueInInt)) return;

      const newTotal = valueInInt * DurInSec;
      const newCost = (((newTotal) / 10) * rate);
      const deltaTotal = newTotal - oldTotal;
      const deltaCost = newCost - oldCost;

      const totalConsumedBalance = calculateTotalConsumedBalance(formState, DealLineItemNo);
      const totalValue = calculateTotalValuSep(formState, name);

      let projectedBalance;
      if (DealLineItemTypeCode === 6 || DealLineItemTypeCode === 5 || DealLineItemTypeCode === 4 || DealLineItemTypeCode === 1) {
        projectedBalance = totalValue + valueInInt;
      } else {
        projectedBalance = totalConsumedBalance + deltaTotal;
      }

      const isWithinLimit = projectedBalance <= limit;
      const isWithinAmt = !isPaymentCheck || deltaCost <= AvailableAmt;

      if (isWithinLimit && isWithinAmt) {
        updateFormStateWithNewValue(name, valueInInt, DurInSec, totalKey, currentDate, Name);
        if (isPaymentCheck) {
          setAvailableAmt((prevAmt) => prevAmt - deltaCost);
        }
      } else {
        openNotification(
          'info',
          !isWithinAmt
            ? 'Booking not allowed due to insufficient available amount.'
  ...
  const getLimit = (DealLineItemTypeCode, NTCBalanceSpots, BalancedSpots, BalancedSeconds) => {
    return DealLineItemTypeCode === 4
      ? NTCBalanceSpots
      : DealLineItemTypeCode === 5 || DealLineItemTypeCode === 6 || DealLineItemTypeCode === 1
        ? BalancedSpots
        : BalancedSeconds;
  };
  ...
  const calculateTotalConsumedBalance = (formState, DealLineItemNo) => {
    const allRowsDataKeys = Object.keys(formState).filter(
      (key) => key[1] === DealLineItemNo.toString() && key[0] === 'Y' && !key.includes('total'),
    );
    return allRowsDataKeys.reduce((total, key) => total + formState[key].total, 0);
  };
  ...
  const calculateTotalValuSep = (formState, name) => {
    const relevantKeys = Object.keys(formState).filter(
      (key) => key.startsWith('Y') && !key.endsWith('|total') && key !== `Y${name}`,
    );
    return relevantKeys.reduce((sum, key) => sum + formState[key].value, 0);
  };

  const updateFormStateWithNewValue = (name, valueInInt, DurInSec, totalKey, currentDate, Name) => {
    setFormState((prevFormState) => {
      ...
      updateFormState(newFormState, currentDate.getFullYear(), currentDate, Name);
      return newFormState;
    });
  };
  ...
  const createInputFields = (
    ...
    for (let currentDate = new Date(startDate); currentDate <= endDate; currentDate.setDate(currentDate.getDate() + 1)) {
      ...
      const isWeekendDay = weekendsdetail.some((weekend) => weekend.Day === currentDay);

      if (trafficDayList.length !== 0) {
        const isActiveDate = trafficDayList.some(
          (item) => item.date === convertDateToYMD(currentDate) && item.IsActive === 1,
        );
        if (!isActiveDate) {
          continue;
        }
      ...
      ((dateFieldIndex) => {
        const fieldName = `${DealLineItemNo}|${commId}|${formatDate(currentDate).replace(/\s+/g, '-')}`;
        ...
        fields.push(
          <input
            ...
            onKeyDown={(e) => handleKeyDownNavigation(e, rowIndex, dateFieldIndex, totalDays)}
            ref={(el) => {
  ...
  useEffect(() => {
    ...
    checkedItems.forEach((dealLineItem) => {
      ...
      formState.Commercial?.forEach((item) => {
        ...
        dates.forEach((date) => {
          const key = `${yearKey}|${commercialKey}|${formatDate(new Date(date))}`;
          newResult[yearKey][commercialKey].push({ [key]: formState[key]?.value || '' });
        });
      });
    });
    setResult(newResult);
  ...
  function checkArray(arr) {
    const firstThreeAreNumbers = arr.slice(0, 2).every((element) => !isNaN(element) && element !== '-');
    const hasNumbersAfterIndexThree = arr.slice(2).some((element) => !isNaN(element) && element !== '-');
      ...
      Object.keys(result).forEach((yearKey) => {
        Object.keys(result[yearKey]).forEach((commercialCode) => {
          ...
          result[yearKey][commercialCode].forEach((BookingValue) => {
            Temp_string +=
              BookingValue[Object.keys(BookingValue)[0]] === '' ? '-,' : BookingValue[Object.keys(BookingValue)[0]] + ',';
          });
          const key = Object.keys(result[yearKey][commercialCode][0])[0];
          const number = key.split('|')[0][1];
          if (checkArray(Temp_string.split(',').filter((item) => item !== ''))) {
            setCheckConditionForCommId(true);
            setCommId(commercialCode);
            setDealId(number);
            setInsettedData(Temp_string.split(',').filter((item) => item !== '' && item !== '-'));
          }
        });
      });
    }
                ...
                <div key={`${checkedIndex}-${commIndex}`}>
                  {checkedIndex === 0 && commIndex === 0 && (
                    <div className="flex" style={{ marginTop: 6 }}>
                      <p style={{ width: 150, fontSize: '10px' }}>TimeBand/Program</p>
                  ...
                  <div className="flex mt-1 items-center">
                    ...
                    <Tooltip title={comm.CommercialCaption}>
                      <p style={{ width: 100, fontSize: '11px', marginTop: 2 }} className="flex items-center">
                        {comm.VideoID}
                        <IoMdInformationCircleOutline className="text-gray-200 ml-1 text-xs" />
                      </p>
                    </Tooltip>
                    <p style={{ width: 60, fontSize: '11px', marginTop: 2 }}>{comm.DurInSec}</p>
                    <p style={{ width: 60, fontSize: '11px', marginTop: 2 }}>
                      {formState[`${DealLineItemNo}|${comm.value}|total`] || '0'}
                    </p>
                    <p style={{ width: 40, fontSize: '11px', marginTop: 2 }}>
                      {((formState[`${DealLineItemNo}|${comm.value}|total`] || 0) * comm.DurInSec).toString()}
                    </p>
                      ...
                      <div className="flex">
                        <Tooltip title="Repeat Pattern">
                          <Button
                            ...
                            icon={<PiRepeat />}
                            onClick={() => {
                              if (!date1 || !date2 || !InsettedData.length) return;
                              const startDate = new Date(date1);
                              const endDate = new Date(date2);
                              const updatedState = { ...formState };
                              let totalSum = 0;
                              const limit = DealLineItemTypeCode === 4 ? NTCBalanceSpots : BalancedSeconds;
                              const allRowsDataKeys = Object.keys(formState).filter(
                                (key) => key[1] === DealLineItemNo.toString() && key[0] === 'Y' && !key.includes('total'),
                              );
                              let totalConsumedBalance = 0;
                              allRowsDataKeys.forEach((key) => {
                                totalConsumedBalance += updatedState[key].total;
                              });

                              const totalKey = `${DealLineItemNo}|${comm.value}`;
                              const oldTotalValue = calculateTotalValue(formState, totalKey);
                              const currentItem = checkedItems.find((item) => item.DealLineItemNo === DealLineItemNo);
                              const rate = currentItem?.Rate || 0;
                              const oldCost = (((oldTotalValue * comm.DurInSec) / 10) * rate); // Updated cost calculation

                              let newTotalValue = 0;
                              let newCost = 0;

                              for (
                                let currentDate = new Date(startDate), i = 0;
                                currentDate <= endDate;
                                ...
                                const formattedDate = formatDate(currentDate).replace(/\s+/g, '-');
                                ...
                                delete updatedState[`${DealLineItemNo}|${comm.value}|total`];

                                const value = Number(InsettedData[i % InsettedData.length]);
                                const total = value * comm.DurInSec;
                                const projectedSum = totalSum + value;
                                const projectedCost = newCost + (((value * comm.DurInSec) / 10) * rate); // Updated cost calculation

                                if (projectedSum * comm.DurInSec <= limit && (!isPaymentCheck || projectedCost <= AvailableAmt + oldCost)) {
                                  updatedState[`Y${fieldNameBase}`] = { value, date: formattedDate, total };
                                  updatedState[fieldNameBase] = value;
                                  totalSum = projectedSum;
                                  newTotalValue += value;
                                  newCost += (((value * comm.DurInSec) / 10) * rate); // Updated cost calculation
                                } else {
                                  break;
                                }
                              }
                              updatedState[`${DealLineItemNo}|${comm.value}|total`] = totalSum;

                              const deltaCost = newCost - oldCost;
                              if (isPaymentCheck && deltaCost > AvailableAmt) {
                                openNotification('info', 'Booking not allowed due to insufficient available amount.');
                                return;
                              }

                              setFormState(updatedState);
                              const stringFromArray = JSON.stringify(transformData(updatedState)).replace(/[%{}]/g, '');
                              setThe_data_I_want_To_Upload(stringFromArray);
                              if (isPaymentCheck) {
                                setAvailableAmt((prev) => prev - deltaCost);
                              }
                            }}
                          />
                        </Tooltip>
                        <Tooltip title="Reset Pattern">
                          <Button
                            ...
                            onClick={() => {
                              ...
                              const currentItem = checkedItems.find((item) => item.DealLineItemNo === DealLineItemNo);
                              const rate = currentItem?.Rate || 0;
                              const costToAddBack = (((oldTotalValue * comm.DurInSec) / 10) * rate); // Updated cost calculation
                              ...
                              if (isPaymentCheck) {
                                setAvailableAmt((prev) => prev + costToAddBack);
                              }
                            }}
                          />
                        </Tooltip>
                    ...
                    <div className="flex items-center" style={{ maxWidth: window.innerWidth - 1200, marginTop: 0 }}>
                      {createInputFields(
                        DealLineItemNo,
                        BalancedSeconds,
                        BalancedSpots,
                        ...
                        <React.Fragment key={inputIndex}>
                          <div>
                            {checkedIndex === 0 && commIndex === 0 && (
                              <p style={{ fontSize: '10px' }}>{inputField.props.label}</p>
                            )}
                            {inputField}
                          </div>
                        </React.Fragment>
