...
const CommercialMasterEdit = forwardRef((props, ref) => {
  ...
  /* STATES */
  const IsNTCPage = (Path == '/NTCCommercialMaster' || Path == '/NTCCommercialMasterEdit') ? true : false;
  //console.log("IsNTCPage:", IsNTCPage)
    ...
    if (emptyFields.length === 0) {
      const mergedData = {
        commercialmaster: {
          ...
          CommercialTypeCode:
            IsNTCPage ? 1 : formState.CommercialType,
          NTCTypeCode:
            IsNTCPage ? 0 : formState.NTCTypeCode,
          TCIN: formState.TCIN,
