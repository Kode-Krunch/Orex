...
function EditNTCDialog({ isDialogOpen, setIsDialogOpen, row, editRowInfo }) {
  ...
  const [gap, setGap] = useState(row.DefaultGAP || row.Gap);
  ...
  const getEventEndTime = (row, isIncludeGap) => {
    return isIncludeGap
      ? addTimes(row.Start_Time, row.Duration, row.DefaultGAP || row.Gap)
  ...
  const durErrorMsg =
    duration.length === 11 &&
    addTimes(offsetTime, duration) > ntcParentRow.Duration
      ? `NTC Duration cannot be greater than its parent's duration`
      : '';
  const offsetTimeErrorMsg =
    offsetTime.length === 11 ? validateOffsetTime(offsetTime) : '';
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setOffsetTime(getFormattedTime(event, offsetTime))
                  }
