...
import NTCSchedulingPage from './NTCSchedulingPage';
...
const NTCScheduling = () => {
  ...
  const DatainPromo = useSelector((state) => state.auth.scheduling.DatainN);
  const timestore = useSelector((state) => state.auth.scheduling.timestoreC);
  const datestore = useSelector((state) => state.auth.scheduling.datestoreC);
  ...
  useEffect(() => {
    ...
    dispatch(setdateForm([currentDate, 'NTC Scheduling']));
    if (gboxElement) {
  ...
  useEffect(() => {
    ...
    apiGetDailyFPCStatus(data, TelecastDate)
      .then((response) => {
        setevents(transformData(response.data));
      })
      .catch((error) => {
  ...
  const onCellSelect = (event, Condition) => {
    setIsOpen(false);
    if (Channel != null) {
      apiGetNTCScheduling(Channel, convertDateToYMD(event.start))
        ...
        .then((data) => {
          ...
          let resdt = updateStartTimes(data, true, false, 'NTC');
          if (Condition) {
          ...
          (async (values) => {
            try {
              const resp = await apiGETColumnSetting('NTC');
              if (resp.status == 204) {
          ...
          (async (values) => {
            try {
              const resp = await apiGETColumnSetting('NTCLeft');
              if (resp.status == 204) {
        ...
        >
          ...
          >
            ...
            <span
              ...
              onClick={() => {
                let res = [...table1Data];
                let res2 = res.filter((row) => row.F_C_S_P != 'NTC');
                setTable1Data(res2);
              }}
            >
              {Channel.label}
