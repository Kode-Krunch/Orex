...
const FPCEdit = () => {
  ...
  useEffect(() => {
    if (vTelecastDate) {
      if (channel && channel.ChannelCode) {
        const fetchLastDayTelecastEndTime = async () => {
          try {
            const reps = await apiCallstoreprocedure('USP_GetFPCEndTime', {
              par_LocationCode: channel.LocationCode,
              par_ChannelCode: channel.ChannelCode,
              par_TeleCastDat: vTelecastDate,
            });

            console.log('API Response:', reps);
            ...
            if (typeof endTime === 'string') {
              setChannelStartTime(endTime.substring(0, 5)); // Extract HH:MM format
            } else {
          ...
          updatedData =
            updt.length > 0
              ? [getDummyRow(updt[0]), ...updt].map((row, index) => ({
                ...
                rowIndex: index,
                cellClassNames: getCellClassNames(updt, row, arr),
                verify: false,
              }))
              : [];
        }
  ...
  useEffect(() => {
    ...
    const savdata = addNewTelecastDate(primaryTableData, SelectedDate);
  ...
  const handleVerifyClick = async () => {
    try {
      ...
      console.log(
        'USP_PG_WeeklyFPC_ValidationvalidationResponse',
        validationResponse,
      );
      const formattedData = [
      ...
      console.log('USP_PG_WeeklyFPC_Validation', formattedData);
  ...
  const handleSaveClick = async (
    ...
    try {
      if (isShowLoader) setShowLoader(true);
      ...
      if (channel.label === CLIENT.USA_Forbes) {
        try {
          setemptySegmentData([]);
          transformedData = addNewTelecastDate(transformedData, SelectedDate);
    ...
    try {
      ...
      let selectedRows =
        sourceDroppableId === droppableIdsEnum.PRIMARY
          ? [data10[sourceIndex]]
          : getUniqueObjects(
            [...secTableSelectedRows, secTableData[sourceIndex]],
            'rowId',
          );
  ...
  const ModifyContent = async () => {
    ...
    if (isEndTimeValid(updt, channelSetting)) {
      maintainScrolledOffsetOfTablesFn();
      setdata10((prevData) => {
        const dt = [...updt];
        let updatedDatax =
          dt.length > 0
            ? [getDummyRow(dt[0]), ...dt].map((row, index) => ({
              ...
              rowId: getRowId('primary', row, 'ContentCode'),
              rowIndex: index,
              cellClassNames: getCellClassNames(dt, row, arr, true),
              verify: false,
            }))
            : [];
        return updatedDatax;
    ...
    // else openNotification('danger', 'Insufficient FPC Duration');
    setVisible(false);
    setSelectedContentToSearch(null);
    setSelContent({ value: null, label: 'Please Select Content' });
    setSelSeason(null);
        ...
        let updatedt =
          newData.length > 0
            ? [getDummyRow(newData[0]), ...newData].map((row, index) => ({
              ...
              rowId: getRowId('primary', row, 'ContentCode'),
              rowIndex: index,
              cellClassNames: getCellClassNames(newData, row, arr, true),
              verify: false,
            }))
            : [];
        return updatedt;
  ...
  const apexBarChar = async (transformedData, condition) => {
    try {
      ...
      transformedArray = addNewTelecastDate(transformedArray, SelectedDate);
  ...
  const addNewTelecastDate = (data, SelectedDate) => {
