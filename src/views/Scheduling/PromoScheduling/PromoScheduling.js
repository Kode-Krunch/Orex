...
import PromoSchedulingPage from './PromoSchedulingPage';
import { hideStackedSideNav_secondary, updateStartTimes } from '../general';
...
const PromoScheduling = () => {
  ...
  const DatainPromo = useSelector((state) => state.auth.scheduling.DatainPromo);
  const timestore = useSelector((state) => state.auth.scheduling.timestore);
  const datestore = useSelector((state) => state.auth.scheduling.datestore);
  ...
  useEffect(() => {
    ...
    dispatch(setdateForm([currentDate, 'Promo Scheduling']));
    if (gboxElement) {
  ...
  useEffect(() => {
    ...
    apiGetDailyFPCStatus(data, TelecastDate)
      .then((response) => {
        setevents(transformData(response.data, 'promoScheduling'));
      })
      .catch((error) => {
  ...
  const onCellSelect = (event, Condition) => {
    setIsOpen(false);
    if (Channel != null) {
      apiGetPromoScheduling(Channel, convertDateToYMD(event.start))
        ...
        .then((data) => {
          ...
          (async (values) => {
            try {
              const resp = await apiGETColumnSetting('Promo');
              if (resp.status == 204) {
          ...
          (async (values) => {
            try {
              const resp = await apiGETColumnSetting('PromoLeft');
              if (resp.status == 204) {
...
export default PromoScheduling;
