        ...
        <Link
          to={
            channel.label == CLIENT.USA_Forbes
              ? '/addtemplate'
              : path == '/EventTeamPlaner' || path == 'EventTeamPlaner'
              ? '/addEventTeamPlaner'
              : '/addcontent'
          }
        >
...
const Contentmaster = () => {
  ...
  const columns = useMemo(
    () => [
...

      {
        ...
        cell: (props) => {
          const row = props.row.original;
          const releaseDate = formatDateToDDMMMYYYY(
            new Date(row.FPCReleaseDate),
          );
          ...
          return (
            <Tooltip
              title={isRecent ? 'Recently Added Event' : 'Previous Added Event'}
            >
              <div className={`flex items-center `}>
                <span
                  className={`ml-2 rtl:mr-2 ${
                    isRecent ? 'text-yellow-500 font-extrabold' : ''
                  }`}
                >
  ...
  }, [channel, path, reloadPageCounter]);
          ...
          <div className="grid grid-cols-4 gap-4">
            {data.map((content, key) => (
              <div className="" key={key}>
                <Card
                  className={`${
                    typeof streamedPercent === 'number' && 'rounded-bl-none'
                  }`}
                  bodyClass={'p-3'}
                >
                  <div
          ...
          <div className="flex flex-col h-full justify-between">
            ...
            {(() => {
              switch (DialogboxName) {
                ...
                case 'Synopsis':
                  return (
                    <SynopsisDialogbox GetContents={GetContents} path={path} />
                  );
                ...
                case 'StarCastEdit':
                  return (
                    <StarCastEditDialog GetContents={GetContents} path={path} />
                  );
