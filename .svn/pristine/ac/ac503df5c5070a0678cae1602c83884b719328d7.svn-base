...
import { loginTypeEnums } from 'views/auth/SignIn/enum';
...
function useAuth() {
  ...
  const signIn = async (values) => {
    try {
      ...
      if (resp.data) {
        if (resp.data.is2FAAllowed) {
          return { status: loginTypeEnums.GOOGLE_TWO_WAY_QR_AUTH };
        } else {
          setLoginInformationInRedux(resp.data, values);
  ...
  const setLoginInformationInRedux = async (loginInfo, values) => {
    const {
      access_token,
      LoginCode,
      PasswordExpiry,
