...
import { handleRemoveRows, TABLE_NAMES } from '../../constants';
import Loader from 'views/Controls/Loader';
import { apiCallstoreprocedure } from 'services/CommonService';
import { format } from 'date-fns';
import RotationInfo from './components/RotationInfo/RotationInfo';
import { getUpdatedSelectedRowsForShiftClick } from './components/DraggableTable/utils';
...
const RemoveRowsDialog = lazy(() =>
  import('./components/DraggableTable/Cell/components/RemoveRowsDialog'),
);
...
function SchedulingArea() {
  ...
  const handleKeyDown = (event) => {
    if (event.key === 'Delete' && schedulingTableSelectedRows.length > 0) {
      handleRemoveRows({
        rows: schedulingTableSelectedRows,
        executeOperation,
        setIsRemoveRowsDialogOpen,
      });
    } else if (event.ctrlKey && event.key.toLowerCase() === 'c')
      handleCopyRows();
    else if (event.ctrlKey && event.key.toLowerCase() === 'x') handleCutRows();
    else if (event.ctrlKey && event.key.toLowerCase() === 'v')
      handlePasteRows();
    else if (event.ctrlKey && event.shiftKey && event.key.toLowerCase() === 'z')
      handleRedo();
    else if (event.ctrlKey && event.key.toLowerCase() === 'z') handleUndo();
    else if (event.key.toLowerCase() === 'arrowdown' && event.shiftKey)
      handleShiftDownSelection();
    else if (event.key.toLowerCase() === 'arrowup' && event.shiftKey)
      handleShiftUpSelection();
    else if (event.key.toLowerCase() === 'arrowdown') handleDownNavigation();
    else if (event.key.toLowerCase() === 'arrowup') handleUpNavigation();
  };
  ...
  const handleCopyRows = () => {
    const copiedEvents = schedulingTableSelectedRows.filter(
      (row) => row.F_C_S_P !== rowDataTypesEnum.COMMERCIAL,
    );
    setSchTableCutCopyMode(cutCopyModeEnum.COPY);
    setSchTableCutCopyRows(copiedEvents);
    openNotification('info', `${copiedEvents.length} events copied`);
    maintainScrolledOffsetOfTables();
  };

  const handleCutRows = () => {
    setSchTableCutCopyMode(cutCopyModeEnum.CUT);
    setSchTableCutCopyRows([...schedulingTableSelectedRows]);
    openNotification(
      'info',
      `${schedulingTableSelectedRows.length} events selected`,
    );
    maintainScrolledOffsetOfTables();
  };

  const handlePasteRows = () => {
    if (schTableCutCopyRows.length === 0)
      return openNotification('info', 'No events to paste');
    else if (!leftClickedSchTableRow)
      return openNotification('info', 'Please select a target event to paste');
    executeOperation({ operation: operationTypesEnum.PASTE_EVENTS });
    maintainScrolledOffsetOfTables();
  };

  const handleShiftDownSelection = () => {
    let nextRow;
    if (!leftClickedSchTableRow) nextRow = schedulingTableData[1];
    else nextRow = schedulingTableData[leftClickedSchTableRow.rowIndex + 1];
    if (!nextRow) return;
    const updatedSelectedRows = getUpdatedSelectedRowsForShiftClick(
      nextRow,
      schedulingTableSelectedRows.reverse(),
      [...schedulingTableData],
      tableTypesEnum.SCHEDULING,
      activeFeatures[featuresEnum.MANAGE_SEGMENT],
    );
    if (schedulingTableRef?.current) {
      schedulingTableRef.current.scrollToItem(nextRow.rowIndex, 'auto');
      setTimeout(() => {
        maintainScrolledOffsetOfTables();
        setSchedulingTableSelectedRows(updatedSelectedRows);
        setLeftClickedSchTableRow(nextRow);
      }, 0);
    }
  };
  ...
  const handleShiftUpSelection = () => {
    let prevRow;
    if (!leftClickedSchTableRow) prevRow = schedulingTableData[1];
    else prevRow = schedulingTableData[leftClickedSchTableRow.rowIndex - 1];
    if (!prevRow) return;
    const updatedSelectedRows = getUpdatedSelectedRowsForShiftClick(
      prevRow,
      schedulingTableSelectedRows.reverse(),
    ...
    if (schedulingTableRef?.current) {
      schedulingTableRef.current.scrollToItem(prevRow.rowIndex, 'auto');
      setTimeout(() => {
        maintainScrolledOffsetOfTables();
        setSchedulingTableSelectedRows(updatedSelectedRows);
        setLeftClickedSchTableRow(prevRow);
      }, 0);
    }
  };
