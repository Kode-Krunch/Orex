...
function BillPrinting() {
  ...
  const [billsDashboardGlobalFilter, setBillsDashboardGlobalFilter] =
    useState('');
  ...
  const getBaseFilterTableData = async (type) => {
    try {
      ...
      if (response.status === 200) {
        ...
        //return response.data.filter(item => item.AgencyCode == 51);

        if (Username === 'GroupMUser') {
          return response.data.filter((item) => item.AgencyCode == 51);
        } else {
  ...
  const getBaseFilterTableData_rev = async (type) => {
    try {
      let param = {
        location: channel.LocationCode,
        channel: channel.ChannelCode,
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-3">
          {baseFilterTables && bills && (
            <DebouncedInput
              value={billsDashboardGlobalFilter ?? ''}
              className="lg:w-52"
              marginBottom="mb-0"
              size="sm"
              prefix={<HiOutlineSearch className="text-lg" />}
              placeholder="Search"
              onChange={(value) => setBillsDashboardGlobalFilter(String(value))}
            ...
            <>
              ...
              {baseFilterTables && bills ? (
                <BillsDashboard
                  ...
                  globalFilter={billsDashboardGlobalFilter}
                  setGlobalFilter={setBillsDashboardGlobalFilter}
                />
