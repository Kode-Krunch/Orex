...
import { executeSecNTCDrag } from './utils/ntcOperations/ntcDrag';
...
const SchedulerContextProvider = ({ children }) => {
  ...
  /* DROPPED NTC STATE */
  const [droppedNtcs, setDroppedNtcs] = useState([]);
  /* LAST MINUTE NTC */
  const [lastMinuteNtcs, setLastMinuteNtcs] = useState([]);
  /* UNDO/REDO */
  ...
  const frames = Number(channelSettings?.FramePerSec) || 25;
  ...
  async function executeOperation({
    ...
    try {
      ...
      } else if (operation === operationTypesEnum.REMOVE_ROWS) {
        operationResult = executeRemoveRows(rowsToRemove, isFirstLoad, frames);
      ...
      let newDropBucket = operationResult?.newDropBucket
        ...
        : [...dropBucket];
      let newDroppedNtcs = operationResult?.newDroppedNtcs
        ? operationResult.newDroppedNtcs
        : [...droppedNtcs];
      ...
      if (
        ...
        newDroppedNtcs = finalizedData.newDroppedNtcs;
  ...
  function executeDragRow(
    ...
    try {
      ...
      let newDroppedNtcs = [...droppedNtcs];
      ...
      if (sourceDroppableId === droppableIdsEnum.SECONDARY) {
        if (
          ...
          newSchedulingTableData = result.newSchTableData;
          newSecondaryTableData = result.newSecTableData;
          newDroppedNtcs = result.newSecTableData;
        } else if (
          activeFeatures[featuresEnum.INSERT].eventType ===
  ...
  function executeBulkInsert({
    ...
    } else newSchedulingTableData = false;
  ...
  function executeRemoveRows(rowsToRemove, isFirstLoad) {
    try {
      let newSchedulingTableData = [],
        newSecondaryTableData = [],
        newDropBucket = [],
        newDroppedNtcs = [];
      /* DROP COMMERCIALS */
      let commToDrop = rowsToRemove.filter(
        (row) => row.F_C_S_P === rowDataTypesEnum.COMMERCIAL,
      );
      let dropCommResult = dropCommercials(
        commToDrop,
        schedulingTableData,
        isFirstLoad,
        frames,
      );
      newDropBucket = dropCommResult.newDropBucket;
      /* REMOVE DELATABLE ROWS AND DROP PAID NTCS */
      let updatedSchTableData = dropCommResult.newSchedulingTableData;
      let newRowsToRemove = rowsToRemove.filter(
        (row) => row.F_C_S_P !== rowDataTypesEnum.COMMERCIAL,
      );
      let removeRowsResult = removeRowsOtherThanComm(
        newRowsToRemove,
        updatedSchTableData,
      );
      newSchedulingTableData = removeRowsResult.newSchedulingTableData;
      newDroppedNtcs = removeRowsResult.newDroppedNtcs;
      /* UPDATE SECONDARY TABLE DATA */
      if (activeFeatures[featuresEnum.INSERT]?.isActive) {
        if (
          activeFeatures[featuresEnum.INSERT].eventType ===
          secondaryTableTypesEnum.DROPPED_SPOTS
        )
          newSecondaryTableData = dropCommResult.newDropBucket;
        else if (
          activeFeatures[featuresEnum.INSERT].eventType ===
          secondaryTableTypesEnum.DROPPED_NTC
        )
          newSecondaryTableData = removeRowsResult.newDroppedNtcs;
  ...
  function dropCommercials(rowsToDrop, schTableData, isFirstLoad) {
    try {
      let selRowsId = rowsToDrop.map((row) => row.rowId);
      /* REMOVE DROPPED ROW FROM SCHEDULING TABLE */
      let newSchedulingTableData = [...schTableData].filter(
        (row) => !selRowsId.includes(row.rowId),
      );
      let newDropBucket;
      let droppedComm = removeAdditionalInfoFromRows(rowsToDrop);
      if (Array.isArray(dropBucket) && dropBucket.length > 0) {
        newDropBucket = [...dropBucket, ...droppedComm];
      } else
        newDropBucket = droppedComm[0]
          ? [getDummyRow(droppedComm[0]), ...droppedComm]
          : [];
      ...
      return {
        newSchedulingTableData: newSchedulingTableData,
        newDropBucket,
      };
    } catch (error) {
      throw error;
  ...
  function removeRowsOtherThanComm(rowsToRemove, tableData) {
    let newSchedulingTableData = [],
      newDroppedNtcs = [];
    /* REMOVE ROWS */
    let selectedRowsId = rowsToRemove.map((row) => row.rowId);
    if (isAutoCalcNtcOffsetTime) {
      ...
      newSchedulingTableData = [...tableData].filter(
        (row) => !selectedRowsId.includes(row.rowId),
      );
    }
    /* PUT ROWS IN DROPPED NTC BUCKET */
    let lDroppedNtcs = rowsToRemove.filter(
      (row) => row.BookingDetailsID || row.BookingDetailID,
    );
    lDroppedNtcs = removeAdditionalInfoFromRows(lDroppedNtcs);
    newDroppedNtcs = putDroppedNtcInNtcBucket(lDroppedNtcs);
    return {
      newSchedulingTableData,
      newDroppedNtcs,
    };
  ...
  function putDroppedNtcInNtcBucket(pDroppedNtcs) {
    /* APPEND DROPPED NTCs TO NTC DROP BUCKET */
    let newDroppedNtcs;
    if (Array.isArray(droppedNtcs) && droppedNtcs.length > 0)
      newDroppedNtcs = [...droppedNtcs, ...pDroppedNtcs];
    else
      newDroppedNtcs = pDroppedNtcs[0]
        ? [getDummyRow(pDroppedNtcs[0]), ...pDroppedNtcs]
        : [];
    return newDroppedNtcs;
  }
    ...
    schedulingTableData.forEach((row) => {
      if (deleteEventTypes.includes(row.F_C_S_P)) {
        if (row.F_C_S_P === rowDataTypesEnum.COMMERCIAL)
          ...
          newDroppedNtcs.push(row);
      } else newSchedulingTableData.push(row);
    });
    if (newDropBucket.length > 0)
      newDropBucket = [getDummyRow(newDropBucket[0]), ...newDropBucket];
    if (newDroppedNtcs.length > 0) {
      newDroppedNtcs = setNtcPropsToZero(newDroppedNtcs);
      newDroppedNtcs = [getDummyRow(newDroppedNtcs[0]), ...newDroppedNtcs];
    }

    return { newSchedulingTableData, newDropBucket, newDroppedNtcs };
  };
    ...
    try {
      ...
      for (let index = 0; index < schedulingTableData.length; index++) {
        ...
        if (contentSegmentRowIds.includes(curRow.rowId)) {
          ...
          // Update row index, row id and sequence no before inserting NTCs
          newSchedulingTableData = getTableDataWithAdditionalInfo({
  ...
  const handleUndo = () => {
    try {
      ...
      const currentState = {
        ...
        droppedNtcs: Array.isArray(droppedNtcs)
          ? [...droppedNtcs]
          : droppedNtcs,
      ...
      setDroppedNtcs(lastUndoState.droppedNtcs);
  ...
  const handleRedo = () => {
    try {
      ...
      setDroppedNtcs(lastRedoState.droppedNtcs);
  ...
  function updateUndoRedoStacksForNewOperation() {
    setUndoStack((prevState) => [
      ...prevState,
      {
        ...
        droppedNtcs: Array.isArray(droppedNtcs)
          ? [...droppedNtcs]
          : prevState.length > 0 && [prevState.length - 1].droppedNtcs
          ? [prevState.length - 1].droppedNtcs
          : null,
        schTableSelectedFilters: [...schTableSelectedFilters],
        secTableSelectedFilters: [...secTableSelectedFilters],
        customCellColor: { ...customCellColor },
