...
import {
  ...
  apiGetTeamMasterDropLocalEventWise,
...
function SelectEntityDialog({
  ...
  const setSelectedEntityListFromAPI = async () => {
    console.log('selectedEvent?.IsSubProgram', selectedEvent?.IsSubProgram);
    ...
    try {
      ...
      if (selectedEvent?.ContentTypeCode === entityTypeCodeEnum.CONTENT) {
        ...
        console.log('IsSubProgram', selectedEvent?.IsSubProgram);
        console.log('ContentCode', selectedEvent?.ContentCode);

        // response = await apiGetTeamMasterDropLocalEventWise(selectedEvent?.IsSubProgram);
        // console.log('response', response);

        const param = {
          ContentCode: selectedEvent?.ContentCode,
          IsSubProgram: selectedEvent?.IsSubProgram,
        };

        console.log('param', param);

        // Call the stored procedure instead of the API method
        response = await apiCallstoreprocedure('USP_GetEventMapTeam', param);
        console.log('response', response);
      ...
      if (response.status === 200) {
        ...
        openNotification(
          'danger',
          `Unable to fetch ${selectedEvent?.ContentTypeCode === entityTypeCodeEnum.CONTENT ? 'Contents' : 'Teams'
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-4">
        <h5>
          {selectedEvent?.ContentTypeCode === entityTypeCodeEnum.CONTENT ? 'Contents' : 'Teams'}
        </h5>
        <DebouncedInput
          value={globalFilter ?? ''}
