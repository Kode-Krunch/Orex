...
const FINAL_LOG_FEATURES = {
  ...
  [featuresEnum.IMPORT_PROMOS]: true,
  [featuresEnum.INSERT_NTC_PATTERN]: true,
  [featuresEnum.VERIFY_COMM]: true,
  [featuresEnum.ADD_PROGRAM]: true,
};
...
const ROW_ACTIONS = {
  REMOVE_ROWS: {
    ...
    actionHandler: ({ rows, executeOperation, setIsRemoveRowsDialogOpen }) => {
      rows.length === 1
        ? executeOperation({
          operation: operationTypesEnum.REMOVE_ROWS,
          rowsToRemove: rows,
        })
        : setIsRemoveRowsDialogOpen(true);
    },
    tooltip: 'Delete',
  },
  ROTATION_INFO: {
...
const TABLE_NAMES = {
  ...
  SECONDARY_TABLE_NAMES: {
    ...
    INSERT_DROPPED_NTC: 'InsertDroppedNTCDraggableTable',
...
const PLAYOUT_EXPORT_FORMAT = {
  ...
  [CLIENT.KINA_U]: playoutExportFormatEnum.CSV,
  [CLIENT.ANI_PLUS]: playoutExportFormatEnum.CSV,
  [CLIENT.SATHIYAM_TV]: playoutExportFormatEnum.XML,
  [CLIENT.Cricbuzz1]: playoutExportFormatEnum.CSV,
  [CLIENT.Cricbuzz2]: playoutExportFormatEnum.CSV,
  [CLIENT.Cricbuzz3]: playoutExportFormatEnum.CSV,
};
...
const handleRemoveRows = ({
  ...
  else if (rows.length > 1) setIsRemoveRowsDialogOpen(true);
};
