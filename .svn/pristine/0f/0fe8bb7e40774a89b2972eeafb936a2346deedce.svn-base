  ...
  try {
    ...
    if (response.status === 200) {
      if (response.data.length === 0) {
        ...
        droppedSpots = removeRowsPresentInTable(
          response.data,
          schedulingTableData,
          'BookingDetailID',
        );
...
function removeRowsPresentInTable(droppedSpots, schedulingTableData, key) {
  try {
    const values = new Set(schedulingTableData.map((row) => row[key]));
    values.delete(null);
    const filteredSpots = droppedSpots.filter((spot) => !values.has(spot[key]));
...
function getRowWithActions(row, channel) {
  try {
    if (
      ...
      return {
        ...row,
        actions: [ROW_ACTIONS.REMOVE_ROWS, ROW_ACTIONS.ROTATION_INFO],
      };
    else if (row.F_C_S_P === rowDataTypesEnum.NTC) {
      if (channel.label === CLIENT.USA_Forbes)
        return {
          ...row,
          actions: [ROW_ACTIONS.REMOVE_ROWS, ROW_ACTIONS.EDIT_ROW],
        };
      else {
...
function finalizeData({
  ...
  /* FINALIZE DROPPED NTCs */
  if (Array.isArray(newDroppedNtcs))
    if (newDroppedNtcs.length < 2) newDroppedNtcs = [];
    else
      newDroppedNtcs = getTableDataWithAdditionalInfo({
        tableData: newDroppedNtcs,
        tableType: tableTypesEnum.SECONDARY,
        additionalInfo: [
          additionalRowInfoEnum.ROW_ID,
          additionalRowInfoEnum.ROW_INDEX,
