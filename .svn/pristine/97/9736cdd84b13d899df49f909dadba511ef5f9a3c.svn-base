...
import EntityDeatilsMasterEdit from './EntityDeatilsMasterEdit';
...
const EntityDeatilsMaster = () => {
        ...
        cell: (props) => {
          ...
          return (
            <div className="flex items-center">
              <span className="ml-2 rtl:mr-2 ">{row.Country?.CountryName}</span>
            </div>
          );
        },
      },
  ...
  useEffect(() => {
    ...
    (async (values) => {
      const Entity = await apiGetEntitymaster(values);
      const formattedOptions = Entity.data.map((option) => ({
        value: option.EntityCode,
        label: option.EntityName,
      }));
      setEntity(formattedOptions);
    })();
    (async (values) => {
      const Place = await apiGetPlaceDrop(values);
  ...
  return (
    <>
      ...
      <Card
        header={<HeaderExtra Component={'Entity Deatils'} />}
      ...
      >
        <DisplayTable
          ...
          ExportName={'EntityDeatils'}
        />
      </Card>

      <Drawer
        title={
          editData.EntityDeatilsName ? (
...
export default EntityDeatilsMaster;
