...
import VerifyCommDialog from './components/VerifyCommDialog/VerifyCommDialog';
...
const AddProgramDialog = lazy(() =>
  import('./components/AddProgramDialog/AddProgramDialog'),
);
...
function Toolbar() {
  ...
  const [isVerifyCommDialogOpen, setIsVerifyCommDialogOpen] = useState(false);
  ...
  const handleSave = () => {
    try {
      ...
      if (channel.label === CLIENT.USA_Forbes) {
        ...
        saveResult = handleSaveScheduleOnlyForbes({
          filteredData,
          setShowLoader,
          dispatch,
          unsavedWork,
      ...
      if (saveResult) setIsUnsavedChangesPresent(false);
    } catch (error) {
  ...
  function getInsertDropdownTotal() {
    try {
      let total = 0;
      const totalDroppedSpots =
        dropBucket.length > 0 ? dropBucket.length - 1 : 0;
      const totalLastMinuteSpots =
        lastMinuteSpots.length > 0 ? lastMinuteSpots.length - 1 : 0;
      const totalDroppedNtcs =
        droppedNtcs.length > 0 ? droppedNtcs.length - 1 : 0;
      if (page === pagesEnum.COMMERCIAL) {
        total = totalDroppedSpots + totalLastMinuteSpots;
      } else if (page === pagesEnum.NTC) {
        total = totalDroppedNtcs;
      } else if (page === pagesEnum.FINAL_LOG) {
        total = totalDroppedSpots + totalLastMinuteSpots + totalDroppedNtcs;
      } else total = 0;
      if (total > 9) total = '9+';
      return total;
    } catch (error) {
  ...
  const insertNextNTCsForSelContent = async () => {
    try {
      setShowLoader(true);
      await executeOperation({
        operation: operationTypesEnum.INSERT_NEXT_NTCS,
      });
      openNotification('success', 'Next NTCs inserted successfully');
    } catch (error) {
      console.error(error);
      openNotification(
        'danger',
        'Something went wrong while inserting Next NTCs for selected program',
      );
    } finally {
      setShowLoader(false);
          ...
          <div className="grow flex justify-between items-center pr-2 mr-2.5 border-r border-r-gray-600">
            <div className="flex items-center gap-0.5">
              ...
              {featuresList[featuresEnum.DURATION] && (
                <Tooltip title="Show Break Wise Duration">
                  <Button
                    className={getClassNames(
                ...
                <>
                  ...
                  {isInsertNextNTCPatternDialogOpen && (
                    <WarningDialog
                      ...
                      submitButtonTitle="Insert"
                      handleDialogSubmit={insertNextNTCsForSelContent}
                      handleDialogClose={() =>
                        setIsInsertNextNTCPatternDialogOpen(false)
                      }
                    />
              ...
              {featuresList[featuresEnum.VERIFY_COMM] && (
                <>
                  <Tooltip title="Verify Commercials">
                    <Button
                      className={getClassNames()}
                      variant="plain"
                      size="sm"
                      icon={
                        <FaBullhorn className="text-[0.98rem] text-gray-200" />
                      }
                      onClick={() => setIsVerifyCommDialogOpen(true)}
                    />
                  </Tooltip>
                  {isVerifyCommDialogOpen && (
                    <VerifyCommDialog
                      isOpen={isVerifyCommDialogOpen}
                      setIsOpen={setIsVerifyCommDialogOpen}
                    />
                  )}
                </>
              )}
              {featuresList[featuresEnum.ADD_PROGRAM] && (
                <>
                  <Tooltip title="Add Program">
                    <Button
                      ...
                      variant="plain"
                      size="sm"
                      icon={<TbLayoutGridAdd className="text-xl" />}
                  ...
                  {isAddProgramDialogOpen && (
                    <AddProgramDialog
                      ...
                      setIsOpen={setIsAddProgramDialogOpen}
                    />
                  )}
                </>
              )}
