...
import { MdDeleteOutline, MdOutlineContentCopy, MdStar } from 'react-icons/md';
...
const ButtonShow = ({
  ...
  /* STATES */
  const [isDeleteContDialogOpen, setIsDeleteContDialogOpen] = useState(false);

  /* EVENT HANDLERS */
  const handleDeleteContent = async () => {
    try {
      setShowLoader(true);
      const response = await apiCallstoreprocedure('GetContentDelete', {
        par_ContentCode: row.original.ContentCode,
      });
      if (Array.isArray(response.data) && response.data.length > 0) {
        if (response.data[0].in_Status === 1) {
          openNotification('info', response.data[0].in_Result);
          setReloadPageCounter((prev) => prev + 1); // re-rendering component
        } else openNotification('danger', response.data[0]?.in_Result);
      } else
        openNotification(
          'danger',
      ...
      console.error(error);
      openNotification('danger', 'Something went wrong while deleting content');
    } finally {
      setIsDeleteContDialogOpen(false);
      setShowLoader(false);
    }
  };
    ...
    >
      <center>
        {(Channel.label === CLIENT.VIJAY_TAKKER ||
          Channel.label === CLIENT.ASIANET || Channel.label === CLIENT.KINA_U) && (
            <Tooltip title="Delete">
              <span
                className={`cursor-pointer pl-3 text-base hover:text-${themeColor}-${primaryColorLevel}`}
                onClick={() => setIsDeleteContDialogOpen(true)}
              >
                <MdDeleteOutline className="text-lg" />
              </span>
              <WarningDialog
                isDialogOpen={isDeleteContDialogOpen}
                title="Delete Content"
                description={
                  <>
                    Are you sure you want to delete{' '}
                    <strong>{row.original.ContentName}</strong> ?
                  </>
                }
                submitButtonTitle="Delete"
                submitButtonColor="red-500"
                submitButtonSize="sm"
                handleDialogSubmit={handleDeleteContent}
                handleDialogClose={() => setIsDeleteContDialogOpen(false)}
              />
            </Tooltip>
          )}
        <Tooltip
