import { Button, Checkbox, Dialog, FormContainer } from 'components/ui';
...
import { apiMaster, apiMasterGet, apiSignIn } from 'services/AuthService';
import SectionsContainer from './SectionsContainer';
import { countries_city } from './ConstantData';
...
import { MdAccessTime, MdOutlineAttachEmail } from 'react-icons/md';

const { unAuthenticatedEntryPath } = appConfig;

const LandingPage = ({ setDisplayLandingPage }) => {
  const nav = useNavigate();
  const dispatch = useDispatch();
  const [isTryDialogOpen, setIsTryDialogOpen] = useState(false);
  const [isAdminDialogOpen, setIsAdminDialogOpen] = useState(false);
  const [isAfterSubTaken, setIsAfterSubTaken] = useState(false);
  ...
  const handleSignInClick = () => {
    nav(unAuthenticatedEntryPath);
    setDisplayLandingPage(true);
  };
  const CreateDBUser = async (data) => {
    setshowLoader(true);
    try {
      const res = await apiMaster({ ...data, PublicIp: ip });
      if (res.status == 200) {
        openNotification(
          'success',
          'Please wait for approval. The details will be sent to your email shortly.',
        );
        setIsAfterSubTaken(true);
        setTimeout(() => { }, 2000); // Simulate loading time
      } else if (res.status == 204) {
        setIsAfterSubTaken(true);
        openNotification(
          'warning',
          'Something went wrong. Please reach out to our team for assistance',
        );
      }
      setshowLoader(false);
      setIsTryDialogOpen(false);
    } catch (error) {
  ...
  // Function to handle step changes with a loading effect
  const handleStepChange = (newStep) => {
    setLoading(true); // Show the loader
    // setTimeout(() => {
    setStep(newStep); // Change the step after a delay
  ...
  const fetchCityPopulation = async () => {
    const city = countries_city.map((item) => ({
      value: item.city,
      label: item.city,
      country: item.country,
    }));
    const country = [
      ...new Set(countries_city.map((item) => item.country)),
    ].map((country) => ({
      value: country,
      label: country,
    }));
  ...
  const signIn = async (values) => {
    ...
    try {
      ...
      setshowLoader(false);
      dispatch(onSignInSuccessAdmin(access_token));
  ...
  const today = dayjs();
  const dateIn7Days = today.add(formData.ExpiredOn, 'day');
  ...
  return (
    <>
      ...
      {!token && (
        <SectionsContainer
          setIsTryDialogOpen={setIsTryDialogOpen}
      ...
      <Dialog
        ...
        contentClassName="dark:!bg-gray-800 !bg-white"
        isOpen={isAfterSubTaken}
        width={500}
        height={550}
        onClose={() => {
          setDisplayLandingPage(false);
          setIsAfterSubTaken(false);
          setFormData({
            ClientName: '',
            ClientCode: 'PMSL',
            ClientType: '',
      ...
      >
        ...
        <h3
          className="py-2 px-5 rounded bg-[#a39ec6] mt-2"
          style={{ opacity: '0.9' }}
        >
          <h5>Amount</h5>
          <h4>$ {formData.price}.00</h4>
        </h3>
        ...
        <div className="flex items-center justify-between mt-2 ">
          <p className="flex items-center">
            <GrChannel className="mr-2" />
            Channel Name
          </p>
          <p>: {formData.ChannelName}</p>
        </div>
        <div className="flex items-center justify-between mt-2 ">
          <p className="flex items-center">
        ...
        <div className="flex items-center justify-between mt-2 ">
          <p className="flex items-center">
            <MdAccessTime className="mr-2" /> Expires on
          </p>
          <p>: {formattedDate}</p>
        </div>
        <div className="mt-2">
      ...
      <Dialog
        ...
        contentClassName="dark:!bg-gray-900 !bg-white rounded-lg"
      ...
      >
        <div className="flex items-center justify-center bg-gray-900 text-white">
          <div className="flex w-12/12  rounded-lg overflow-hidden shadow-lg">
            {/* Left Section */}
              ...
              <div className="mb-2">
                <div>
                  <div className="flex items-center mb-5">
                    <div className="primaryLanding flex items-center justify-center rounded-[14px]">
                    ...
                    <div>
                      <h4 className="ml-4 text-xl font-bold text-dark dark:text-white">
                        Cloud Platform
                      </h4>
                      <p className="text-body-color dark:text-dark-6 ml-4 ">
                        Access your broadcast scheduling system from anywhere
                        with our secure, scalable cloud platform, ensuring
                        real-time data availability and seamless performance.
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center mb-5 ">
                    <div
                      className="primaryLanding flex items-center
                    ...
                    <div>
                      ...
                      <p className="text-body-color dark:text-dark-6 ml-4 ">
                        ...
                        insights into scheduling patterns, optimize airtime
                        usage, and boost revenue through smart decision-making.
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center mb-5 ">
                    ...
                    <div>
                      ...
                      <p className="text-body-color dark:text-dark-6 ml-4 ">
                        ...
                        designed to streamline your broadcast scheduling
                        workflow.
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center mb-5 ">
                    ...
                    <div>
                      ...
                      <p className="text-body-color dark:text-dark-6 ml-4 ">
                        Compatible with various playout systems, accounting
                        tools like Tally and SAP, and external integrations,
            ...
            {/* Right Section */}
            <div className="w-1/2 p-8 bg-gray-700/50 rounded text-gray-700">
              <h2 className="text-2xl font-bold mb-2">
                Sign Up for a <span className="text-teal-500">Company</span>
              </h2>
                    ...
                    <FormContainer>
                        ...
                        <div>
                          ...
                          <div className="mt-2 mb-2">
                            ...
                            <Button
                              ...
                              disabled={!(values.Address.length > 20)}
                              onClick={() => CreateDBUser(values)}
                              variant="solid"
                              size="sm"
                            >
                              Sign Up
                      ...
                      {step === 1 && (
                        <div>
                          ...
                          <Button
                            type="submit"
                            className="w-full  text-white p-2  mt-2"
                            // disabled={!(values.Address.length > 10)}
