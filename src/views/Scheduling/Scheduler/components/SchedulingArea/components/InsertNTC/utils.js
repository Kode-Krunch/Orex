...
import { removeRowsPresentInTable } from 'views/Scheduling/Scheduler/context/utils';
...
const getNTCs = async (
  ...
  try {
    ...
    const response = await apiCallstoreprocedure('USP_Sch_Get_All_NTC', {
      ...
      NTCName: searchInputValue ? searchInputValue : '%%',
    ...
    if (response.status === 200) {
      if (response.data.length === 0) {
        ...
        tableData =
          ntcType === 'paid'
            ? // Remove NTCs present in scheduling table and dropped NTCs table
        ...
        tableData =
          tableData.length === 0
            ? []
            : [getDummyRow(tableData[0]), ...tableData].map((row, index) => {
        ...
        if (tableData.length === 0) openNotification('info', 'No NTCs found');
      }
    } else if (response.status === 204) {
      openNotification('info', 'No NTCs found');
    } else {
