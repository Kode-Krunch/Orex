...
import { apipostbookingdetails, apipostbookingdetailswithTime } from 'services/DealServices';
import {
...
function RescheduleSpotsDialog({ isOpen, setIsOpen, selRows, resetData }) {
  ...
  const handleDateChange = async (date) => {
    setDate(date);
    console.log('selRows[0]', selRows[0]);
  ...
  const handleConfirm = async () => {
    const finalData = selRows.map((item) => ({
      ...
      ScheduleTime: `${time.value}:00:00`,
    }));
    try {
      const resp = await apipostbookingdetailswithTime('RESCHEDULE', finalData, 0);
      if (resp.status === 200) {
        setIsOpen(false);
        resetData();
