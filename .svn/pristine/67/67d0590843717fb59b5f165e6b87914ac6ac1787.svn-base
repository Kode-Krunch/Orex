  ...
  const handleImport = async (files) => {
    try {
      ...
      reader.onload = async (event, index) => {
        ...
        // else if (fileExt === 'csv' && channel.label === CLIENT.NEWS)''AmitG
        else if (fileExt === 'csv' && channel.label?.toUpperCase() === 'NEWS CHANNEL INDIA') {
          const text = new TextDecoder().decode(arrayBuffer);
          ...
          lines.slice(2).forEach((line) => {
            ...
            if (values.length < 3 || values[3] === 'FileName') return;

            tableData.push({
              //Time: values[0].includes('-') ? values[0].split('-')[1] : '',
              Time: values[1],
              FileName: values[2],
              Title: values[3],
              ActualDuration: values[5],
              Status: 0,
              Date: values[0],
            });
          });
